...
public class IceAndSnowNoBorealeneFeature extends Feature<NoFeatureConfig> {
    public IceAndSnowNoBorealeneFeature(Codec<NoFeatureConfig> p_i231993_1_) {
        super(p_i231993_1_);
    }
        ...
        for(int i = 0; i < 16; ++i) {
            for(int j = 0; j < 16; ++j) {
                ...
                if (biome.doesSnowGenerate(p_230362_1_, blockpos$mutable)) {
                    if (p_230362_1_.getBlockState(blockpos$mutable1).getBlock() != FrostburnExpansionBlocks.BOREALENE.get()) {
